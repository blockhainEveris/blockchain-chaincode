...
func (t *SimpleChaincode) vote(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	// votingId, optionId, justification, senderid, name, channel, boteid, conversationid
	//     0        1      	     2         3    			4      5	    6	     			7
	// ["1", "2", "Justification", "1", "Peter", "Web", "bote10", "conv10"]
	...
	// 4th argument - senderId
	if len(args[3]) <= 0 {
		return nil, errors.New("4th argument must be a non-empty string")
	}

	// 6th argument - channel
	if len(args[5]) <= 0 {
			...
			if allvotings.Votings[i].Status == false {
				...
				if currentTime > expirationTime {
					...
					for j := range allvotings.Votings[i].Voters {
						if allvotings.Votings[i].Voters[j].SenderId == args[3] {
					...
					// Add new member
					newMember := Member{}
					newMember.SenderId = args[3]
					newMember.Name = args[4]
					newMember.Channel = args[5]
					newMember.BoteId = args[6]
					newMember.ConversationId = args[7]
